...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        distanceLabel.frame = CGRect(x: 50, y: 50, width: 150, height: 25)
        view.addSubview(distanceLabel)

        trackingStateLabel.frame = CGRect(x: 50, y: 80, width: 300, height: 25)
    ...
    @objc func handleTapGesture(sender: UITapGestureRecognizer) {
        ...
        if let endNode = endNode {
            ...
            self.endNode = nil
            distanceLabel.text = ""
            trackingStateLabel.text = lines.count.description + " lines"
            return
        }
        ...
        if let result = planeHitTestResults.first {
            ...
            if let startNode = startNode {
                ...
                distanceLabel.text = "Distance: " + formatter.string(from: NSNumber(value: vector.length()*100))! + "cm"
            }
            else {
                startNode = node
            }
            ...
            if let startNode = startNode {
                endNode = sphereNode
                let a = distance(startNode: startNode, endNode: sphereNode)*100
                self.lines.append(a)
                line = lineBetweenNodes(nodeA: startNode, nodeB: endNode!)
                sceneView?.scene.rootNode.addChildNode(line!)
                self.distanceLabel.text = String(format: "%.2f", a) + "cm"
            }
            else {
    ...
    func lineBetweenNodes(nodeA: SCNNode, nodeB: SCNNode) -> SCNNode {
        return CylinderLine(parent: sceneView!.scene.rootNode, v1: nodeA.position, v2: nodeB.position, radius: 0.0005,radSegmentCount: 16, color: UIColor.red)
    }
